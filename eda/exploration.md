...
1. `eda/sql/district_normalize.sql` — SQL CTE handling formatting noise.
2. `eda/state_aliases.csv` — 3 state-level aliases (NFHS misspells `Maharastra`, uses formal `NCT of Delhi` vs pincode-dir `DELHI`, and merges Dadra+Daman+Diu vs pincode-dir `THE DADRA AND NAGAR HAVELI AND DAMAN AND DIU`). These three rows alone fix 52 districts that were stranded in a cross-state bucket.
3. `eda/district_aliases_auto.csv` (75 rows, generated by `eda/_data/fuzzy_match.py`) + `eda/district_aliases_manual.csv` (66 rows, hand-curated for govt renames like Gurgaon→Gurugram, Allahabad→Prayagraj, the 9 Karnataka renames, multi-word reorderings like `Dakshin Dinajpur`↔`DINAJPUR DAKSHIN`, and one NFHS data-quality fix where `Mizoram/Chandel` is reclassified to `Manipur/Chandel`).

End-to-end verification: `python3 eda/_data/validate_coverage.py` resolves all 142 originally-unmatched NFHS districts using the alias layers.
...
- `state_aliases.csv` — 3 NFHS state-norm → pincode-dir state-norm aliases.
- `district_aliases_auto.csv` — 75 auto-accepted district aliases from `_data/fuzzy_match.py` (difflib.SequenceMatcher ≥ 0.90).
- `district_aliases_manual.csv` — 66 hand-curated district aliases for govt renames, abbreviations, word reorderings, and one NFHS data-quality fix.
- `district_alias_candidates.csv` — full audit trail of every (NFHS, pincode-dir) candidate pair scored by the fuzzy matcher (252 rows).
- `notes/district_alias_review.md` — residual review notes from the fuzzy matcher (post-state-alias residuals that needed human review).
- `sql/district_normalize.sql` — reusable SQL CTE for joining NFHS against the pincode directory after formatting normalization.
- `_data/` — intermediate JSON snapshots (NFHS unmatched, pincode-dir districts) plus the matcher and validator scripts. Reproducible audit trail; safe to delete and regenerate.
